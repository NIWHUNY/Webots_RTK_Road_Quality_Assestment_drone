...
fourcz = cv.VideoWriter_fourcc(*'XVID')  # Choose the codec (e.g., 'XVID', 'MJPG') based on the video format you want
out = cv.VideoWriter('output.avi', fourcz, 20.0, (400, 240))  # Create a VideoWriter object
...
while robot.step(8) != -1:
    ...
    cv.imshow("jumpshot gg gaming", np_image)
    if keyboard.getKey() == ord('N'):
        break
    out.write(np_image)
    cv.waitKey(1)
